...
- Focus projections follow only analyzed files. Unsupported files keep the action disabled; external packages are included only when Dependencies is enabled. Meaning has separate Overview, Components, Workflows, Questions and Verified/Authored lenses. Its inspector exposes incoming/outgoing reasoning links with trust, status, confidence and source evidence. It renders provisional workflow entry candidates, but does not claim multi-step runtime order, branches, data flow, lifecycle, dynamic dispatch, or impact without stronger compiler/framework/runtime evidence.
...
| Languages    | Real TS/JS and Python/Pyright IDE services, optional rust-analyzer, Python environment selection and Python/Rust tasks, plus Python/Rust/TS semantic extraction | Rust debugger adapter, resolved cross-language calls, full framework adapters     |
