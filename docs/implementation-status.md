...
- Focus projections follow only analyzed files. Unsupported files keep the action disabled; external packages are included only when Dependencies is enabled. The Meaning view renders provisional workflow entry candidates, but does not yet claim multi-step runtime order, branches, data flow, lifecycle, dynamic dispatch, or impact without stronger compiler/framework/runtime evidence.
