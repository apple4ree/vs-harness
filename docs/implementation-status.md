...
- Focus projections follow only analyzed files. Unsupported files keep the action disabled; external packages are included only when Dependencies is enabled. Workflow, sequence, data-flow and lifecycle views require new authored/observed typed IR and are not synthesized from imports.
