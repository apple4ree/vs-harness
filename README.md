...
| 구조 / AI       | 실제 import 근거를 가진 그래프, 컴포넌트 드래그 첨부, Codex 스트리밍, 격리 편집, 파일별 diff 승인·적용 후 그래프 갱신     |

TS/JS 관계는 TypeScript AST와 모듈 해석을 사용합니다. Python은 제한적인 정적 분석입니다. 확인하지 못한 관계를 AI가 추측해 그래프에 넣지 않습니다. 큰 프로젝트는 20,000개 탐색 항목·64 MB 소스 읽기, 화면은 220개 노드·600개 연결로 제한됩니다. 검색과 모듈 상세 보기로 범위를 좁힐 수 있고, 연결을 선택하면 실제 import 위치와 코드 근거가 표시됩니다.
...
3. 카드의 드래그 손잡이를 오른쪽 채팅에 놓습니다.
4. **Ask**는 질문, **Change · isolated copy**는 분리된 복사본에서 변경을 수행합니다.
5. 변경 검토 화면에서 실제 파일 diff를 확인하고 적용할 파일만 선택합니다. 원본은 승인 후 바뀝니다.
6. 원본 충돌이 있으면 적용을 거부하며, 그래프는 저장된 실제 코드에서 다시 계산됩니다.
